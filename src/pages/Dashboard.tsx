...
import { usePositions } from '../hooks/positions';
import { useUserContext } from '../contexts/UserContext';
import type { JobFormData, TalentAnalysis, Company, CompanyFormData, SavedPosition } from '../types';
...
export function Dashboard() {
  ...
  const [showCompanyForm, setShowCompanyForm] = useState(false);
  const [companyToEdit, setCompanyToEdit] = useState<Company | null>(null);
  const [selectedPosition, setSelectedPosition] = useState<SavedPosition | null>(null);
  const [error, setError] = useState<string | null>(null);
  ...
  const handlePositionSelect = (position: SavedPosition) => {
    ...
    // Find and select the corresponding company
